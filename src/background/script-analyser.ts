...
export function scanScriptForAnomalies(scriptContent: string, scriptId: string): boolean {
    ...
    try {
        ...
        function traverse(node: any): void {
            ...
            // Rule 1: Dangerous calls (eval, atob, etc.) - checks CallExpression nodes
            ...
            // Rule 2: Obfuscation methods (fromCharCode, charCodeAt, etc.) - checks MemberExpression nodes
            ...
            // Rule 3: Cookie modification (potential session theft) - checks AssignmentExpression nodes
            ...
            for (const key in node) {
                // Ensure we only look at own properties and skip metadata (like 'loc')
                if (node.hasOwnProperty(key)) {
                    const child = node[key];
                    if (Array.isArray(child)) {
                        child.forEach(traverse);
                    } else if (typeof child === 'object' && child !== null && child.type) {
                        // Only traverse objects that look like AST nodes (have a 'type' property)
                        traverse(child);
                    }
                }
            }
